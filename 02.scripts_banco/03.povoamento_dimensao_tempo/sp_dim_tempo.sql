		...
		BEGIN
				...
				BEGIN
					INSERT INTO DIM_TEMPO VALUES 
						...
						STR(DATEPART(qq,@dt_inicial)) + 'º ' + 'Trimestre / '+ TRIM(STR(YEAR(@dt_inicial))),
						IIF(MONTH(@dt_inicial ) >= 6 ,2,1),
						STR(IIF(MONTH(@dt_inicial) >= 6 ,2,1)) + 'º' + 'Semestre / '+ TRIM(STR(YEAR(@dt_inicial))),
						YEAR(@dt_inicial))
				...
				INSERT INTO DIM_TEMPO VALUES
									...
									STR(DATEPART(qq,DATEADD(mm,-1,@dt_inicial))) + 'º ' + 'Trimestre / '+ TRIM(STR(@ANO)),
									IIF(MONTH(DATEADD(mm,-1,@dt_inicial)) >= 6 ,2,1),
									STR(IIF(MONTH(DATEADD(mm,-1,@dt_inicial)) >= 6 ,2,1)) + 'º' + 'Semestre /'+ TRIM(STR(@ANO)),
...
EXEC SP_INSERT_DIM_TEMPO '20210101','20221231'
SELECT * FROM DIM_TEMPO